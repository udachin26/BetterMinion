...
class MinionLimiter
{
    ...
    public static function addCount(string $playerName, string $levelName)
    {
        if (!isset(self::$playerMinions[$playerName][$levelName])) self::$playerMinions[$playerName][$levelName] = 0;
        self::$playerMinions[$playerName][$levelName]++;
    }

    public static function reduceCount(string $playerName, string $levelName)
    {
        self::$playerMinions[$playerName][$levelName]--;
    }
    ...
    public static function isFull(string $playerName, string $levelName): bool
    {
        ...
        if ($minionCount >= $maxCount) return true;
        return false;
    }
}
